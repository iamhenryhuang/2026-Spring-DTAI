...
* **第二階段 (Fine-tuning)**：**解凍模型參數 (Unfreeze All Layers)**，以極小的學習率進行全局微調，使模型能更精準地捕捉植物葉片的細微病變特徵，最終將 Accuracy 推升至 **97% 以上**。
...
- **信心度回傳**：輸出疾病類別並附帶 **信心程度 (Confidence Score)**。
- **高效推理**：ResNet-18 的輕量化特性，使其在邊緣設備端也能維持快速反應。

## 未來展望
- **模型壓縮**：考慮導入權重剪枝 (Pruning) 或量化 (Quantization)，進一步減少移動端部署的負擔。
- **多病害檢測**：擴展模型以支援同一張圖中多種病害的同步標註。
